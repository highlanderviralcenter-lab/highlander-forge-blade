...
impl WinServiceProvider { pub fn new() -> Self { Self } }

impl ServiceProvider for WinServiceProvider {
    fn list_all(&self) -> Result<Vec<ServiceInfo>, CoreError> { Ok(Vec::new()) }
    fn get_status(&self, _name: &str) -> Result<String, CoreError> {
        Err(CoreError::NotSupported("Servicos nao implementado".to_string()))
    }
