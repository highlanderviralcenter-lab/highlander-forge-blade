...
impl StateFile {
    pub fn new(machine_id: String) -> Self {
        ...
        let mut s = Self {
            ...
            created_at: now, updated_at: now, machine_id,
            app_version: env!("CARGO_PKG_VERSION").to_string(),
            checksum: String::new(), audit_data: None, cleanup_data: None, phases_executed: Vec::new(),
        };
    ...
    pub fn verify_checksum(&self) -> bool {
        ...
        format!("{:08x}", crc32(&json)) == self.checksum
    }
}
...
pub fn load_state() -> Result<StateFile, StateError> {
    let path = state_path();
    if !path.exists() { return Err(StateError::NotFound); }
    let content = std::fs::read_to_string(&path).map_err(|e| StateError::Io(e.to_string()))?;
    let raw: serde_json::Value = serde_json::from_str(&content).map_err(|e| StateError::Parse(e.to_string()))?;
    let version = raw.get("schema_version").and_then(|v| v.as_u64()).unwrap_or(0) as u32;
    match version {
        0 => migrate_v0_to_v1(raw),
        1 => {
            let state: StateFile = serde_json::from_value(raw).map_err(|e| StateError::Parse(e.to_string()))?;
            if !state.verify_checksum() { return Err(StateError::InvalidChecksum); }
            Ok(state)
        }
...
pub fn save_state(state: &StateFile) -> Result<(), StateError> {
    let path = state_path();
    std::fs::create_dir_all(BASE_DIR).map_err(|e| StateError::Io(e.to_string()))?;
...
fn state_path() -> PathBuf { Path::new(BASE_DIR).join(STATE_FILE) }
...
fn migrate_v0_to_v1(mut raw: serde_json::Value) -> Result<StateFile, StateError> {
    ...
    raw["machine_id"] = serde_json::json!(machine_id::get_or_create_machine_id().map_err(|e| StateError::Io(e.to_string()))?);
    raw["app_version"] = serde_json::json!(env!("CARGO_PKG_VERSION"));
    raw["checksum"] = serde_json::json!("");
    if raw.get("phases_executed").is_none() { raw["phases_executed"] = serde_json::json!(Vec::<String>::new()); }
